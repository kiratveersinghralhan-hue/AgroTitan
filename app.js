...
/* ===== v64 contact enquiry button ===== */
function initContactEnquiry(){
...
/* ===== v72-v76 aggregate features ===== */
function setAuthMessage(id, text, isError=false){ const el=document.getElementById(id); if(!el) return; el.textContent=text||''; el.style.color=isError?'#ffb0a8':'#dccaa1'; }
async function saveContactLead(payload){ return window.HPBackend?.insert ? HPBackend.insert('contactLeads', payload) : null; }
async function saveSellerLead(payload){ return window.HPBackend?.insert ? HPBackend.insert('sellerLeads', payload) : null; }
async function saveCallbackLead(payload){ return window.HPBackend?.insert ? HPBackend.insert('callbacks', payload) : null; }
async function saveBuyerEnquiryV76(payload){ return window.HPBackend?.insert ? HPBackend.insert('buyerEnquiries', payload) : null; }
async function listBuyerEnquiriesV76(){ return window.HPBackend?.list ? HPBackend.list('buyerEnquiries') : []; }
async function initLoginPageV72(){ const btn=document.getElementById('loginSubmitBtn'); const resetBtn=document.getElementById('resetPasswordBtn'); if(!btn||!window.HPAuth) return; const session=await HPAuth.getSession(); if(session&&session.user){ window.location.href='account.html'; return; } if(btn.dataset.bound!=='1'){ btn.dataset.bound='1'; btn.addEventListener('click', async ()=>{ const email=(document.getElementById('loginEmail')?.value||'').trim(); const password=(document.getElementById('loginPassword')?.value||'').trim(); setAuthMessage('authLoginMessage','Logging in...'); const res=await HPAuth.login(email,password); if(res.error){ setAuthMessage('authLoginMessage',res.error.message||'Login failed.',true); return; } setAuthMessage('authLoginMessage','Login successful. Redirecting...'); setTimeout(()=>window.location.href='account.html',500); }); } if(resetBtn&&resetBtn.dataset.bound!=='1'){ resetBtn.dataset.bound='1'; resetBtn.addEventListener('click', async ()=>{ const email=(document.getElementById('loginEmail')?.value||'').trim(); if(!email){ setAuthMessage('authLoginMessage','Enter your email first for password reset.',true); return; } const res=await HPAuth.resetPassword(email); if(res.error){ setAuthMessage('authLoginMessage',res.error.message||'Reset request failed.',true); return; } setAuthMessage('authLoginMessage','Password reset email sent.'); }); } }
async function initSignupPageV72(){ const btn=document.getElementById('signupSubmitBtn'); if(!btn||!window.HPAuth) return; const session=await HPAuth.getSession(); if(session&&session.user){ window.location.href='account.html'; return; } if(btn.dataset.bound!=='1'){ btn.dataset.bound='1'; btn.addEventListener('click', async ()=>{ const full_name=(document.getElementById('signupName')?.value||'').trim(); const email=(document.getElementById('signupEmail')?.value||'').trim(); const password=(document.getElementById('signupPassword')?.value||'').trim(); setAuthMessage('authSignupMessage','Creating account...'); const res=await HPAuth.signup(email,password,full_name); if(res.error){ setAuthMessage('authSignupMessage',res.error.message||'Signup failed.',true); return; } setAuthMessage('authSignupMessage','Account created. Check email if confirmation is enabled, then login.'); }); } }
async function initAccountPageV72(){ const logoutBtn=document.getElementById('accountLogoutBtn'); const emailEl=document.getElementById('accountEmail'); const nameEl=document.getElementById('accountName'); const roleEl=document.getElementById('accountRole'); const idEl=document.getElementById('accountId'); const msgEl=document.getElementById('accountMessage'); if((!emailEl&&!logoutBtn)||!window.HPAuth) return; const user=await HPAuth.getUser(); if(!user){ window.location.href='login.html'; return; } const profile=await HPAuth.getProfile(); if(msgEl) msgEl.textContent=profile&&profile.role==='admin'?'Admin account detected. You can access the protected admin dashboard.':'Signed in successfully.'; if(emailEl) emailEl.textContent=user.email||'—'; if(nameEl) nameEl.textContent=(profile&&(profile.full_name||profile.public_name))||(user.user_metadata&&user.user_metadata.full_name)||'—'; if(roleEl) roleEl.textContent=(profile&&profile.role)||'user'; if(idEl) idEl.textContent=user.id||'—'; if(logoutBtn&&logoutBtn.dataset.bound!=='1'){ logoutBtn.dataset.bound='1'; logoutBtn.addEventListener('click', async ()=>{ await HPAuth.logout(); window.location.href='login.html'; }); } }
async function enhanceHeaderAuthStateV72(){ if(!window.HPAuth) return; const user=await HPAuth.getUser(); document.querySelectorAll('.auth-nav-actions').forEach(box=>{ if(box.dataset.bound==='1') return; box.dataset.bound='1'; box.innerHTML=user?'<a class="badge-btn" href="account.html">My Account</a>':'<a class="badge-btn" href="login.html">Login</a><a class="badge-btn" href="signup.html">Sign Up</a>'; }); }
function showBackendStatusV69(){ const note=document.getElementById('backendStatusNote'); if(!note) return; const configured=window.HPBackend&&window.HPBackend.isSupabaseConfigured&&window.HPBackend.isSupabaseConfigured(); note.textContent=configured?'Supabase mode is enabled. Leads will sync to your Supabase tables when the project URL, anon key, and table schema are valid.':'Supabase mode is selected, but config placeholders are still present. Replace backend-config.js values to make the backend live. Until then, local fallback storage is used.'; }
let sellerImageDataV74=[];
function normalizeImagesV74(item){ if(Array.isArray(item.image_urls)) return item.image_urls; if(typeof item.image_urls==='string'){ try{ const parsed=JSON.parse(item.image_urls); return Array.isArray(parsed)?parsed:[]; }catch(e){ return []; } } return []; }
function previewSellerImagesV74(files){ const box=document.getElementById('sellerImagePreview'); if(!box) return; box.innerHTML=''; const arr=Array.from(files||[]).slice(0,3); sellerImageDataV74=[]; arr.forEach(file=>{ const reader=new FileReader(); reader.onload=function(e){ sellerImageDataV74.push(e.target.result); const div=document.createElement('div'); div.className='seller-image-thumb'; div.innerHTML=`<img src="${e.target.result}" alt="Seller machine photo">`; box.appendChild(div); }; reader.readAsDataURL(file); }); }
function initSellerImageUploadV74(){ const input=document.getElementById('sellerImages'); if(!input||input.dataset.bound==='1') return; input.dataset.bound='1'; input.addEventListener('change', function(){ previewSellerImagesV74(this.files); }); }
async function uploadSellerImagesToSupabaseV75(files,userId){ if(!window.HPSupabase||!window.HPSupabase.client||!files?.length) return {urls:[],paths:[]}; const client=window.HPSupabase.client; const urls=[]; const paths=[]; for(const file of Array.from(files).slice(0,3)){ const safeName=(Date.now()+'-'+Math.random().toString(36).slice(2)+'-'+file.name).replace(/[^a-zA-Z0-9._-]/g,'-'); const path=`${userId||'anonymous'}/${safeName}`; const {error}=await client.storage.from('seller-images').upload(path,file,{upsert:false}); if(!error){ const {data}=client.storage.from('seller-images').getPublicUrl(path); urls.push(data.publicUrl); paths.push(path); } } return {urls,paths}; }
async function bindSellerLeadButtonsV76(){ const sellerBtn=document.getElementById('sellerWhatsAppBtn'); const callbackBtn=document.getElementById('sellerCallbackBtn'); if(!sellerBtn||sellerBtn.dataset.v76bound==='1'||!window.HPAuth||!window.HPBackend) return; sellerBtn.dataset.v76bound='1'; const user=await HPAuth.getUser(); sellerBtn.addEventListener('click', async function(e){ e.preventDefault(); let uploaded={urls:sellerImageDataV74||[],paths:[]}; const fileInput=document.getElementById('sellerImages'); const hasRealSupabase=!!(window.HPSupabase&&window.HPSupabase.client&&window.HP_BACKEND_CONFIG?.provider==='supabase'); if(fileInput?.files?.length&&hasRealSupabase&&user?.id){ try{ uploaded=await uploadSellerImagesToSupabaseV75(fileInput.files,user.id); }catch(err){ console.warn('Storage upload failed, falling back to local previews.',err);} }
const lead={ source:'seller-center', user_email:user?.email||'', user_id:user?.id||'', name:document.getElementById('sellerName')?.value||'', phone:document.getElementById('sellerPhone')?.value||'', brand:document.getElementById('sellerBrand')?.value||'', model:document.getElementById('sellerModel')?.value||'', year:document.getElementById('sellerYear')?.value||'', hours:document.getElementById('sellerHours')?.value||'', location:document.getElementById('sellerLocation')?.value||'', price:document.getElementById('sellerPrice')?.value||'', notes:document.getElementById('sellerNotes')?.value||'', channel:'seller-whatsapp', listing_stage:'pending', image_urls:uploaded.urls?.length?uploaded.urls:(sellerImageDataV74||[]), image_paths:uploaded.paths||[], is_public:false };
await saveSellerLead(lead); const lines=['Hello Harvester Parts, I want to list my used combine.',lead.name?'Seller: '+lead.name:'',lead.phone?'Phone: '+lead.phone:'',lead.brand?'Brand: '+lead.brand:'',lead.model?'Model: '+lead.model:'',lead.year?'Year: '+lead.year:'',lead.hours?'Hours: '+lead.hours:'',lead.location?'Location: '+lead.location:'',lead.price?'Expected Price: '+lead.price:'',lead.notes?'Condition / Notes: '+lead.notes:'',lead.image_urls?.length?'Photos attached on dashboard.':''].filter(Boolean); window.open('https://wa.me/'+SHOP.whatsapp+'?text='+encodeURIComponent(lines.join('
')),'_blank'); alert('Seller lead saved with images. You can track and edit it in Seller Dashboard.'); }); if(callbackBtn&&callbackBtn.dataset.v76bound!=='1'){ callbackBtn.dataset.v76bound='1'; callbackBtn.addEventListener('click', async function(){ await saveCallbackLead({ source:'seller-center', user_email:user?.email||'', user_id:user?.id||'', name:document.getElementById('sellerName')?.value||'', phone:document.getElementById('sellerPhone')?.value||'', requirement:((document.getElementById('sellerBrand')?.value||'')+' '+(document.getElementById('sellerModel')?.value||'')).trim(), status:'seller-sales-call' }); alert('Sales callback request saved.'); }); } }
async function renderSellerDashboardV74(){ const grid=document.getElementById('sellerDashboardGrid'); if(!grid||!window.HPAuth||!window.HPBackend) return; const user=await HPAuth.getUser(); if(!user){ window.location.href='login.html'; return; } const allLeads=await HPBackend.list('sellerLeads'); const myLeads=(allLeads||[]).filter(item=>((item.user_email&&item.user_email===user.email)||(item.user_id&&item.user_id===user.id))); const totalEl=document.getElementById('sellerListingsCount'); const pendingEl=document.getElementById('sellerPendingCount'); const approvedEl=document.getElementById('sellerApprovedCount'); if(totalEl) totalEl.textContent=String(myLeads.length); if(pendingEl) pendingEl.textContent=String(myLeads.filter(x=>(x.listing_stage||'pending')!=='approved').length); if(approvedEl) approvedEl.textContent=String(myLeads.filter(x=>(x.listing_stage||'')==='approved').length); if(!myLeads.length){ grid.innerHTML='<div class="card seller-empty">No seller listing requests found for this account yet. Create one from Seller Center.</div>'; return; } grid.innerHTML=myLeads.map(item=>{ const images=normalizeImagesV74(item); return `<article class="card seller-listing-card"><h4>${item.brand||'Used Combine'} ${item.model||''}</h4><div class="seller-listing-meta">${item.year?`<span>${item.year}</span>`:''}${item.location?`<span>${item.location}</span>`:''}${item.status?`<span>${item.status}</span>`:''}${item.listing_stage?`<span>${item.listing_stage}</span>`:''}</div>${images.length?`<div class="seller-gallery">${images.slice(0,3).map(src=>`<img src="${src}" alt="Listing photo">`).join('')}</div>`:''}<p>${item.notes||'Seller submission saved and waiting for further review.'}</p><div class="helper">${item.price?'Expected Price: '+item.price:''}</div><div class="seller-listing-actions"><button class="primary-btn" type="button" onclick="openSellerEditModalV74('${item.id}')">Edit Request</button><button class="ghost-btn" type="button" onclick="deleteSellerLeadV73('${item.id}')">Delete Request</button></div></article>`; }).join(''); }
async function deleteSellerLeadV73(id){ if(!window.HPBackend) return; await HPBackend.remove('sellerLeads', id); renderSellerDashboardV74(); renderUsedCombinesPageV76(); }
async function openSellerEditModalV74(id){ const modal=document.getElementById('sellerEditModal'); if(!modal||!window.HPBackend) return; const leads=await HPBackend.list('sellerLeads'); const item=(leads||[]).find(x=>x.id===id); if(!item) return; document.getElementById('editSellerId').value=item.id||''; document.getElementById('editSellerBrand').value=item.brand||''; document.getElementById('editSellerModel').value=item.model||''; document.getElementById('editSellerYear').value=item.year||''; document.getElementById('editSellerLocation').value=item.location||''; document.getElementById('editSellerPrice').value=item.price||''; document.getElementById('editSellerHours').value=item.hours||''; document.getElementById('editSellerNotes').value=item.notes||''; modal.classList.add('open'); modal.setAttribute('aria-hidden','false'); }
function closeSellerEditModalV74(){ const modal=document.getElementById('sellerEditModal'); if(!modal) return; modal.classList.remove('open'); modal.setAttribute('aria-hidden','true'); }
async function saveSellerEditV74(){ const id=document.getElementById('editSellerId')?.value; if(!id||!window.HPBackend) return; await HPBackend.update('sellerLeads', id, { brand:document.getElementById('editSellerBrand')?.value||'', model:document.getElementById('editSellerModel')?.value||'', year:document.getElementById('editSellerYear')?.value||'', location:document.getElementById('editSellerLocation')?.value||'', price:document.getElementById('editSellerPrice')?.value||'', hours:document.getElementById('editSellerHours')?.value||'', notes:document.getElementById('editSellerNotes')?.value||'' }); closeSellerEditModalV74(); renderSellerDashboardV74(); renderUsedCombinesPageV76(); }
async function loadSellerProfileFormV75(){ const saveBtn=document.getElementById('saveSellerProfileBtn'); if(!saveBtn||!window.HPAuth||!window.HPSupabase?.client) return; const user=await HPAuth.getUser(); if(!user) return; const profile=await HPAuth.getProfile(); if(!profile) return; const publicName=document.getElementById('profilePublicName'); const phone=document.getElementById('profilePhone'); const district=document.getElementById('profileDistrict'); const bio=document.getElementById('profileBio'); const avatar=document.getElementById('profileAvatarUrl'); const isPublic=document.getElementById('profileIsPublicSeller'); const viewLink=document.getElementById('viewPublicSellerProfileBtn'); if(publicName) publicName.value=profile.public_name||profile.full_name||''; if(phone) phone.value=profile.phone||''; if(district) district.value=profile.district||''; if(bio) bio.value=profile.bio||''; if(avatar) avatar.value=profile.avatar_url||''; if(isPublic) isPublic.checked=!!profile.is_public_seller; if(viewLink) viewLink.href='seller-profile.html?id='+encodeURIComponent(user.id); }
async function saveSellerProfileV75(){ if(!window.HPAuth||!window.HPSupabase?.client) return; const user=await HPAuth.getUser(); if(!user) return; const client=window.HPSupabase.client; const payload={ public_name:document.getElementById('profilePublicName')?.value||'', phone:document.getElementById('profilePhone')?.value||'', district:document.getElementById('profileDistrict')?.value||'', bio:document.getElementById('profileBio')?.value||'', avatar_url:document.getElementById('profileAvatarUrl')?.value||'', is_public_seller:!!document.getElementById('profileIsPublicSeller')?.checked }; const msg=document.getElementById('sellerProfileMessage'); if(msg) msg.textContent='Saving seller profile...'; const {error}=await client.from('profiles').update(payload).eq('id', user.id); if(error){ if(msg) msg.textContent=error.message||'Could not save seller profile.'; return; } if(msg) msg.textContent='Seller profile saved successfully.'; }
async function initSellerProfileSettingsV75(){ const saveBtn=document.getElementById('saveSellerProfileBtn'); if(!saveBtn) return; await loadSellerProfileFormV75(); if(saveBtn.dataset.bound!=='1'){ saveBtn.dataset.bound='1'; saveBtn.addEventListener('click', saveSellerProfileV75); } }
async function renderPublicSellerProfileV75(){ const nameEl=document.getElementById('publicSellerName'); const listingsGrid=document.getElementById('publicSellerListingsGrid'); if(!nameEl||!listingsGrid||!window.HPSupabase?.client) return; const params=new URLSearchParams(window.location.search); const id=params.get('id'); if(!id){ nameEl.textContent='Seller not found'; return; } const client=window.HPSupabase.client; const {data:profile}=await client.from('profiles').select('*').eq('id',id).single(); if(!profile||!profile.is_public_seller){ nameEl.textContent='Seller profile unavailable'; return; } const avatarBox=document.getElementById('publicSellerAvatar'); const metaEl=document.getElementById('publicSellerMeta'); const bioEl=document.getElementById('publicSellerBio'); nameEl.textContent=profile.public_name||profile.full_name||'Seller Profile'; if(metaEl) metaEl.textContent=[profile.district,profile.phone].filter(Boolean).join(' • ')||'Public seller profile'; if(bioEl) bioEl.textContent=profile.bio||'Approved seller listings are shown below.'; if(avatarBox){ if(profile.avatar_url) avatarBox.innerHTML=`<img src="${profile.avatar_url}" alt="Seller avatar">`; else avatarBox.textContent=(profile.public_name||profile.full_name||'S').trim().charAt(0).toUpperCase(); } const {data:listings}=await client.from('seller_leads').select('*').eq('user_id',id).eq('is_public',true).eq('listing_stage','approved').order('created_at',{ascending:false}); const countEl=document.getElementById('publicSellerListingsCount'); if(countEl) countEl.textContent=String((listings||[]).length)+' listings'; listingsGrid.innerHTML=(listings||[]).length ? listings.map(item=>{ const imgs=normalizeImagesV74(item); return `<article class="card seller-listing-card"><h4>${item.brand||'Used Combine'} ${item.model||''}</h4><div class="seller-listing-meta">${item.year?`<span>${item.year}</span>`:''}${item.location?`<span>${item.location}</span>`:''}<span>Approved</span></div>${imgs.length?`<div class="seller-gallery">${imgs.slice(0,3).map(src=>`<img src="${src}" alt="Seller listing">`).join('')}</div>`:''}<p>${item.notes||'Approved seller listing.'}</p><div class="helper">${item.price?'Expected Price: '+item.price:''}</div></article>`; }).join('') : '<div class="card seller-empty">No public listings available from this seller yet.</div>'; }
async function approveSellerLeadV75(id){ if(!window.HPBackend) return; await HPBackend.update('sellerLeads', id, { listing_stage:'approved', status:'approved', is_public:true }); renderAdminDashboardV76(); renderUsedCombinesPageV76(); }
async function rejectSellerLeadV74(id){ if(!window.HPBackend) return; await HPBackend.update('sellerLeads', id, { listing_stage:'rejected', status:'rejected' }); renderAdminDashboardV76(); }
async function renderAdminDashboardV76(){ const contactGrid=document.getElementById('adminContactGrid'); const sellerGrid=document.getElementById('adminSellerGrid'); const callbackGrid=document.getElementById('adminCallbackGrid'); if(!contactGrid&&!sellerGrid&&!callbackGrid) return; if(!window.HPBackend) return; const [contactLeads,sellerLeads,callbacks]=await Promise.all([HPBackend.list('contactLeads'),HPBackend.list('sellerLeads'),HPBackend.list('callbacks')]); const contactCount=document.getElementById('adminContactCount'); const sellerCount=document.getElementById('adminSellerCount'); const callbackCount=document.getElementById('adminCallbackCount'); if(contactCount) contactCount.textContent=String(contactLeads.length); if(sellerCount) sellerCount.textContent=String(sellerLeads.length); if(callbackCount) callbackCount.textContent=String(callbacks.length); function cardHTML(item,collection){ if(collection==='sellerLeads'){ const images=normalizeImagesV74(item); return `<article class="card admin-lead-card"><h4>${item.name||item.brand||'Lead'}</h4><div class="admin-lead-meta">${item.phone?`<span>${item.phone}</span>`:''}${item.source?`<span>${item.source}</span>`:''}${item.status?`<span>${item.status}</span>`:''}${item.listing_stage?`<span>${item.listing_stage}</span>`:''}</div>${images.length?`<div class="admin-gallery">${images.slice(0,3).map(src=>`<img src="${src}" alt="Seller machine photo">`).join('')}</div>`:''}<p>${item.requirement||item.notes||item.model||'No extra details provided.'}</p><div class="helper">${item.createdAt?new Date(item.createdAt).toLocaleString():''}</div><div class="admin-lead-actions"><button class="primary-btn" type="button" onclick="approveSellerLeadV75('${item.id}')">Approve</button><button class="ghost-btn" type="button" onclick="rejectSellerLeadV74('${item.id}')">Reject</button><button class="ghost-btn" type="button" onclick="deleteLeadV67('sellerLeads','${item.id}')">Delete</button></div></article>`; } return `<article class="card admin-lead-card"><h4>${item.name||item.brand||'Lead'}</h4><div class="admin-lead-meta">${item.phone?`<span>${item.phone}</span>`:''}${item.source?`<span>${item.source}</span>`:''}${item.status?`<span>${item.status}</span>`:''}</div><p>${item.requirement||item.notes||item.model||'No extra details provided.'}</p><div class="helper">${item.createdAt?new Date(item.createdAt).toLocaleString():''}</div><div class="admin-lead-actions"><button class="primary-btn" type="button" onclick="markLeadDoneV67('${collection}','${item.id}')">Mark Done</button><button class="ghost-btn" type="button" onclick="deleteLeadV67('${collection}','${item.id}')">Delete</button></div></article>`; } if(contactGrid) contactGrid.innerHTML=contactLeads.length?contactLeads.map(item=>cardHTML(item,'contactLeads')).join(''):'<div class="card admin-empty">No contact enquiries yet.</div>'; if(sellerGrid) sellerGrid.innerHTML=sellerLeads.length?sellerLeads.map(item=>cardHTML(item,'sellerLeads')).join(''):'<div class="card admin-empty">No seller submissions yet.</div>'; if(callbackGrid) callbackGrid.innerHTML=callbacks.length?callbacks.map(item=>cardHTML(item,'callbacks')).join(''):'<div class="card admin-empty">No callback requests yet.</div>'; }
function closeBuyerEnquiryModalV76(){ const modal=document.getElementById('buyerEnquiryModal'); if(!modal) return; modal.classList.remove('open'); modal.setAttribute('aria-hidden','true'); }
async function openBuyerEnquiryModalV76(listingId,sellerUserId,sellerName,listingTitle=''){ const modal=document.getElementById('buyerEnquiryModal'); if(!modal) return; document.getElementById('buyerEnquiryListingId').value=listingId||''; document.getElementById('buyerEnquirySellerUserId').value=sellerUserId||''; document.getElementById('buyerEnquirySellerName').value=sellerName||''; document.getElementById('buyerEnquiryMessage').value=listingTitle?('I am interested in '+listingTitle+'. Please share more details.'):'I am interested in this used combine. Please share more details.'; modal.classList.add('open'); modal.setAttribute('aria-hidden','false'); if(window.HPAuth){ const user=await HPAuth.getUser(); if(user){ const profile=await HPAuth.getProfile?.(); const emailEl=document.getElementById('buyerEnquiryEmail'); const nameEl=document.getElementById('buyerEnquiryName'); if(emailEl&&!emailEl.value) emailEl.value=user.email||''; if(nameEl&&!nameEl.value) nameEl.value=profile?.full_name||user.user_metadata?.full_name||''; } } }
async function submitBuyerEnquiryV76(){ const payload={ seller_lead_id:document.getElementById('buyerEnquiryListingId')?.value||'', seller_user_id:document.getElementById('buyerEnquirySellerUserId')?.value||'', seller_public_name:document.getElementById('buyerEnquirySellerName')?.value||'', buyer_name:document.getElementById('buyerEnquiryName')?.value||'', buyer_phone:document.getElementById('buyerEnquiryPhone')?.value||'', buyer_email:document.getElementById('buyerEnquiryEmail')?.value||'', message:document.getElementById('buyerEnquiryMessage')?.value||'', status:'new' }; await saveBuyerEnquiryV76(payload); alert('Buyer enquiry sent successfully.'); closeBuyerEnquiryModalV76(); }
async function renderSellerBuyerEnquiriesV76(){ const grid=document.getElementById('sellerBuyerEnquiriesGrid'); if(!grid||!window.HPAuth||!window.HPBackend) return; const user=await HPAuth.getUser(); if(!user){ grid.innerHTML='<div class="card seller-empty">Login required to view buyer enquiries.</div>'; return; } let all=[]; try{ all=await listBuyerEnquiriesV76(); }catch(e){ console.warn(e); } const mine=(all||[]).filter(item=>item.seller_user_id===user.id); grid.innerHTML=mine.length?mine.map(item=>`<article class="card buyer-enquiry-card"><h4>${item.buyer_name||'Buyer enquiry'}</h4><div class="seller-listing-meta">${item.buyer_phone?`<span>${item.buyer_phone}</span>`:''}${item.buyer_email?`<span>${item.buyer_email}</span>`:''}${item.status?`<span>${item.status}</span>`:''}</div><p>${item.message||'Buyer requested more details.'}</p><div class="helper">${item.createdAt?new Date(item.createdAt).toLocaleString():''}</div><div class="seller-listing-actions"><a class="primary-btn" href="https://wa.me/${SHOP.whatsapp}?text=${encodeURIComponent('Hello, I want to respond to buyer enquiry from ' + (item.buyer_name || 'buyer'))}" target="_blank" rel="noopener">Contact Sales</a></div></article>`).join(''):'<div class="card seller-empty">No buyer enquiries on your listings yet.</div>'; }
async function renderUsedCombinesPageV76(){ const grid=document.getElementById('usedCombinesGrid'); if(!grid||!window.USED_COMBINES) return; const search=(document.getElementById('usedCombineSearch')?.value||'').trim().toLowerCase(); const brand=(document.getElementById('usedBrandFilter')?.value||''); const condition=(document.getElementById('usedConditionFilter')?.value||''); const location=(document.getElementById('usedLocationFilter')?.value||'').trim().toLowerCase(); let extra=[]; if(window.HPBackend){ const sellerLeads=await HPBackend.list('sellerLeads'); extra=(sellerLeads||[]).filter(x=>(x.listing_stage||'')==='approved').map((item,idx)=>({ id:item.id||('sup-'+idx), brand:item.brand||'Used Combine', model:item.model||'Seller Listing', year:item.year||'', location:item.location||'', condition:'Verified', price:item.price||'Ask Price', askingPrice:item.price||'Ask Price', description:item.notes||'Approved seller listing.', image:normalizeImagesV74(item)[0]||'https://images.unsplash.com/photo-1500382017468-9049fed747ef?auto=format&fit=crop&w=1400&q=80', badge:'Approved Seller', sellerName:item.name||'', seller_user_id:item.user_id||'', hours:item.hours||'', ownership:'Seller listed', negotiable:true, age:item.year||'' })); }
